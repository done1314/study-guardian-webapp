...
const initialState = {schemaVersion:2,startDate:today,mathDays:DEFAULT_MATH_DAYS,defaults:{math:90,english:30},durations:{},proofs:{},completed:{},customTasks:[],restDays:{},advanceEvents:[]};
let state = loadState();
...
function seriesDefinitions(){
  ...
  return [
    {id:'math',kind:'math',category:'数学',startDate:state.startDate,totalDays:state.mathDays,defaultDuration:state.defaults.math,itemAt:index=>({title:schedule[index].title,detail:`文件夹学习第 ${schedule[index].part} / ${schedule[index].days} 天 · 共 ${schedule[index].count} 项 · 完成后拍照`})},
    {id:'english',kind:'english',category:'英语',startDate:state.startDate,totalDays:ENGLISH_DAYS,defaultDuration:state.defaults.english,itemAt:index=>({title:`完成第 ${index+1} 篇阅读`,detail:'精读一篇文章，标出生词与长难句后拍照'})},
    ...state.customTasks.map(task=>({id:`custom-${task.id}`,kind:'custom',category:task.category,startDate:task.startDate,totalDays:task.totalDays,defaultDuration:task.duration,itemAt:index=>({title:task.title,detail:`${task.category} · 第 ${index+1} / ${task.totalDays} 天 · 完成后拍照`})}))
  ];
}
...
function renderProgress(){
  const range=periodRange(),entries=completedEntries().map(([itemId,entry])=>({itemId,...entry})).filter(entry=>entry.completedDate>=range.start&&entry.completedDate<=range.end),byCategory={};entries.forEach(entry=>{byCategory[entry.category]=(byCategory[entry.category]||0)+Number(entry.minutes||0);});
  const groups=Object.entries(byCategory).sort((a,b)=>b[1]-a[1]),total=groups.reduce((sum,[,minutes])=>sum+minutes,0),days=new Set(entries.map(entry=>entry.completedDate));document.querySelector('#progressDate').value=progressDate;document.querySelector('#focusRange').textContent=range.label;document.querySelector('#focusTotal').textContent=total<1000?total:(total/60).toFixed(1);document.querySelector('#focusPie span').textContent=total<1000?'分钟':'小时';
  let angle=0;const stops=groups.map(([category,minutes],index)=>{const start=angle;angle+=minutes/Math.max(total,1)*100;return `${chartColors[index%chartColors.length]} ${start}% ${angle}%`;});document.querySelector('#focusPie').style.background=groups.length?`conic-gradient(${stops.join(',')})`:'conic-gradient(rgba(118,118,128,.12) 0 100%)';document.querySelector('#focusPie').setAttribute('aria-label',groups.length?groups.map(([category,minutes])=>`${category}${minutes}分钟`).join('，'):'暂无专注数据');
  const legend=document.querySelector('#focusLegend');legend.innerHTML='';groups.forEach(([category,minutes],index)=>{const button=document.createElement('button');button.className='legend-item';button.title=`${category}：${formatMinutes(minutes)}，占 ${Math.round(minutes/total*100)}%`;button.innerHTML=`<span class="legend-dot" style="background:${chartColors[index%chartColors.length]}"></span><strong>${escapeText(category)}</strong><span>${Math.round(minutes/total*100)}%</span>`;const show=()=>document.querySelector('#pieDetail').textContent=`${category}：${formatMinutes(minutes)} · ${Math.round(minutes/total*100)}%`;button.addEventListener('mouseenter',show);button.addEventListener('focus',show);button.addEventListener('click',show);legend.append(button);});if(!groups.length)legend.innerHTML='<div class="custom-empty">暂无数据</div>';document.querySelector('#pieDetail').textContent=groups.length?'悬浮或点击分类，查看具体专注时间。':'完成任务后，这里会按分类统计专注时间。';
  document.querySelector('#statsGrid').innerHTML=`<div class="stat-card"><strong>${formatMinutes(total)}</strong><span>总专注时长</span></div><div class="stat-card"><strong>${entries.length}</strong><span>完成任务</span></div><div class="stat-card"><strong>${days.size}</strong><span>专注天数</span></div>`;
  const history=document.querySelector('#historyList');history.innerHTML='';const all=completedEntries().map(([itemId,entry])=>({itemId,...entry})).sort((a,b)=>b.completedDate.localeCompare(a.completedDate));if(!all.length){history.innerHTML='<div class="empty-history">完成第一次拍照打卡后，记录会出现在这里。</div>';return;}all.slice(0,20).forEach(entry=>{const row=document.createElement('div');row.className='history-row';row.innerHTML=`<span class="history-dot">${icons.check}</span><div><strong>${escapeText(entry.title)}</strong><span>${formatDate(entry.completedDate)} · ${escapeText(entry.category)} · ${formatMinutes(entry.minutes)}</span></div>`;history.append(row);});
}
...
document.querySelector('#saveDuration').addEventListener('click',()=>{state.durations[durationEditing.itemId]=Math.max(5,Math.min(480,Number(document.querySelector('#durationInput').value)||30));saveState();closeSheets();renderToday();});
